...
/* Original credit to FTC 3486
 * TODO: Add press detection */
public class GamepadWrapper
{
    ...
    public double scaleJoystick(float inputThrottle, double scaleFactor) {
        double throttle = inputThrottle;
        // Dead zone
        throttle = (Math.abs(throttle) < 0.1) ? 0 : throttle;
        // Scale from 0 to 1 (or scaleFactor)
        throttle = -(Math.signum(throttle) * ((Math.pow(throttle, 4) * (.9)) + .1) * scaleFactor);
        // Clip at 1: Should never be necessary
